export default function ({ isHMR, app, store, route, params, req, error, redirect }) {
  ...
  if (req) {
    if (route.name) {
      ...
      const availableLanguages = store.getters.availableLanguages
      let lang = availableLanguages.find(item => item.aliases.includes(locale))

      store.commit('SET_LANGUAGE', !lang ? 'en_GB' : lang.iso)
    }
  }
}
